...
public abstract class SPDCharacterController : MonoBehaviour
{
    ...
    protected void InitSettings()
    {
        ...
        spartaAnim = GetComponentInChildren<Animation>();
    ...
    protected void RotateCharacter()
    {
        ...
        {
            Vector3 forward = Vector3.Slerp(characterObj.transform.forward, direction,
                    rotateSpeed * Time.deltaTime / Vector3.Angle(characterObj.transform.forward, direction ) );
            characterObj.transform.LookAt(characterObj.transform.position + forward );
            MoveCharacter();
        }
        else
        {
    ...
    {
        if(other.gameObject.CompareTag("Weapon") && other.transform.root.CompareTag(enemyTag))
        {
            DecreaseHP();
        }
    }
